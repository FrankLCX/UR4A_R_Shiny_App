...
ui <- navbarPage (theme = shinytheme('flatly'),"Pricing Pal",
                  tabPanel("For sellers: Pricing Strategy Tool",
                           sidebarLayout(
                                   sidebarPanel(
                                           ...
                                           #actionButton(inputId = 'button1', label = 'GO')
                                   ),
                                   mainPanel(
                                           htmlOutput('text'),
                                           textOutput('summary'),
                  ...
                  tabPanel('For buyers: Price Comparison Database',
                           sidebarLayout(
                                   sidebarPanel(
                                           selectInput(inputId = 'Brand_name1',
                                                       label = 'Any preference on brands?',
                                                       choices = sort(unique(d$brand)),
                                                       selected = NULL),
                                           selectInput(inputId = 'Shipping1',
                                                       label = 'Shipping covered by seller?',
                                                       choices = c('Yes!','No.'),
                                                       selected = NULL),
                                           selectInput(inputId = 'First_category1',
                                                       label = 'Please select your first category',
                                                       choices = sort(unique(d$FirstCategory)),
                                                       selected = NULL),
                                           selectInput(inputId = 'Second_category1',
                                                       label = 'Please select your second category',
                                                       choices = sort(unique(d$SecondCategory)),
                                                       selected = NULL),
                                           selectInput(inputId = 'Third_category1',
                                                       label = 'Please select your third category',
                                                       choices = sort(unique(d$ThirdCategory)),
                                                       selected = NULL),
                                           numericInput(inputId = 'Price_low',
                                                        label = 'I wish to look for a price above:',
                                                        value=0),
                                           numericInput(inputId = 'Price_high',
                                                        label = 'I wish to look for a price no higher than:',
                                                        value=10000)
                                           #actionButton(inputId = 'button2', label = 'GO')
                                   ),
                                   mainPanel(
                                           tableOutput('retrieved')
                                   )
                           )
...
server <- function(input, output,session) {
        ...
        observe({
                updateSelectInput(session=session, 
                                  inputId = 'Second_category1',
                                  choices = sort(unique(subset(d,FirstCategory==input$First_category1)$SecondCategory)))
        })
        observe({
                updateSelectInput(session=session, 
                                  inputId = 'Third_category1',
                                  choices = sort(unique(subset(d,FirstCategory==input$First_category1
                                                               &SecondCategory==input$Second_category1)$ThirdCategory)))
        })  
        observe({
                updateSelectInput(session=session, 
                                  inputId = 'Brand_name1',
                                  choices = sort(unique(subset(d,FirstCategory==input$First_category1
                                                               &SecondCategory==input$Second_category1
                                                               &ThirdCategory==input$Third_category1)$brand_name)))
        })  
        ...
        output$text<-renderUI({
                str0 <- paste("___________________________________________________________________________________________________")
        ...
        ############################################################################################################
        ##Buyers
        d1<-reactive(d%>%filter(FirstCategory==input$First_category1
                                &SecondCategory==input$Second_category1
                                &ThirdCategory==input$Third_category1
                                &brand_name==input$Brand_name1
                                &price>=input$Price_low
                                &price<=input$Price_high))
        output$retrieved<-renderTable({d1()[,c('name','price','item_condition_id','brand_name','item_description')]})
}
